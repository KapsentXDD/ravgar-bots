...
class Ban extends Command {
    ...
    async onRequest(client, message, args, embed, aris) {
        ...
        if (!member) return message.channel.send({ content: system.Replys.Member + ` \`${system.Bot.Prefixs[0]}ban <<ID>>\`` }).sil(20)
            ...
            collector.on('collect', async (interaction) => {
                ...
                if (interaction.values[0] == "bbir") {
                    ...
                    await Punitives.find({}).exec(async (err, res) => {
                        ...
                        client.channels.cache.find(x => x.name === "ban-log").send({ embeds: [embed.setDescription(`${member} (\`${member.id}\`) kişisi ${message.author} (\`${message.author.id}\`) tarafından **${sebep}** sebebiyle **${tarihsel(Date.now())}** tarihinde ban türü ceza ile cezalandırıldı!`).setFooter({ text: `Developed By Positron • Ceza Numarası: #${ceza.No}` }).setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true, size: 2048 }) }).setColor("Random")] })
            ...
            collector.on('collect', async (interaction) => {
                ...
                if (interaction.values[0] == "bbir") {
                    ...
                    await Punitives.find({}).exec(async (err, res) => {
                        ...
                        if (member && member.manageable) await member.setRoles(aris?.jailedRole).catch(x => client_logger.log("Underworld rolü verilemedi lütfen Rol ID'sini kontrol et.", "caution"));;
                        message.react(message.guild.findEmoji(system.Emojis.Onay))
                    })
                }
                if (interaction.values[0] == "biki") {
