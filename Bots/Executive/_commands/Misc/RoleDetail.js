...
class RoleDetail extends Command {
    ...
    async onRequest(client, message, args, embed, aris) {
        ...
        if (sesteOlmayanlar.length >= 1) message.channel.send(`${sesteOlmayanlar.slice(0, sesteOlmayanlar.length / 1).join(`, `)}`, { code: "diff", split: true }).catch(arisc => {
