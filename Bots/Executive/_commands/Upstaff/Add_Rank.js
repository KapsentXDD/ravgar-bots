    ...
    async onRequest(client, message, args, embed, aris) {
        ...
        if (["ekle", "add"].includes(args[0])) {
            if (!vcoin || isNaN(vcoin)) return message.channel.send({ content: `\`Hata:\` Eklenecek coin miktarını belirtmelisin!` }).sil(100)
                if (aris?.staffRanks.some((x) => x.coin == vcoin)) return message.channel.send({ embeds: [embed.setDescription(`**UYARI :** \`${vcoin}\` coinine ulaşıldığında verilecek roller zaten ayarlanmış!`)] }).sil(30)
                const roles = [...message.mentions.roles.values()];
                if (!roles || !roles.length) return message.reply({ embeds: [embed.setDescription("Eklenecek yetkinin rol(leri) belirtmelisin!")] });
                await ariscik.findOneAndUpdate({ guildID: message.guild.id }, { $push: { staffRanks: { role: roles.map((x) => x.id), coin: parseInt(vcoin) } } }, { upsert: true })
        ...
        if (["sil", "temizle"].includes(args[0])) {
            ...
            await ariscik.findOneAndUpdate({ $pull: { staffRanks: { coin: parseInt(vcoin) } } });
