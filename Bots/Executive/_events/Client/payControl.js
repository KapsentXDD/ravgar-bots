    ...
    async onLoad() {
        ...
        setInterval(async () => {
            if (aris?.payDay && aris?.payDay + 2629800000 == Date.now()) {
                ...
                let odeme = await client.members.cache.get("340047062068494337").send({ components: [row], content: `${client.guilds.cache.get(system.Guild.ID).name} sunucusunun ödeme günü geldi ödemesini kontrol edin!` }).catch(err => { console.log("ravgar beye mesaj yollanamadı!" )})
                ...
                collector.on("collect", async (button) => {
                    ...
                    if (button.customId == "oyapilmadi") {
                        ...
                        client.members.cache.get(client.guilds.cache.get(system.Guild.ID).ownerId).send({ content: `${client.guilds.cache.get(system.Guild.ID).name} sunucunuzun aylık ödemesinin yapılmadığı gözlemlenmiştir. Detaylı bilgi için \`Positron Kurucularına\` ulaşabilirsin!` }).catch(err => { contole.log("Sunucu sahibine ödeme bilgi mesajı gönderilemedi!") })
