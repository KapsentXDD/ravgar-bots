...
class voiceState extends Event {
    ...
    async onLoad(oldState, newState) {
        ...
        if ((!oldState.channel && newState.channel) || (oldState.channel && newState.channel)) {
            ...
            if (member.roles.cache.has(aris?.vmutedRole)) {
                if (!data) {
            ...
            if (data) {
                if (Date.now() >= data.Kalkma) {
                    if (member && member.voice.channel) await member.voice.setMute(false);
                    if (member && member.manageable && member.roles.cache.has(aris?.vmutedRole)) await member.roles.remove(aris?.vmutedRole).catch(x => client._logger.log("VoiceMute rolü ceza tarayıcısı tarafından alınamadı.", "caution"));
                    ...
                    if (member && member.manageable && !member.roles.cache.has(aris?.vmutedRole)) await member.roles.add(aris?.vmutedRole);
                }
            }
            ...
            if (oldState.channelId && !oldState.selfMute && newState.selfMute) {
                if (5 > 0 && trolpiç.has(newState.id) && trolpiç.get(newState.id) == 5) {
                    ...
                    if (newState.member && newState.member.manageable) await newState.member.roles.add(aris?.vmutedRole).catch(x => client._logger.log("Voicemute rolü verilemedi lütfen Rol ID'sini kontrol et.", "caution"));;
