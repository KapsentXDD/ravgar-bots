...
Discord.Guild.prototype.log = async function log(cezano, user, admin, tip, channelName) {
  ...
  if (channel) {
    let embed = new Discord.EmbedBuilder()
      ...
      .setFooter({ text: "Developed By Positron" + ` • Ceza Numarası: #${cezano.No}`, iconURL: client.guild.iconURL({ dynamic: true }) })
...
const spuni = global.spuni = async function (id, type) {
  let uye = Selfb.guilds.cache.get(system.Guild.ID).members.cache.get(id);
  if (!uye) return;
...
const puni = global.puni = async function (id, type) {
  ...
  if (type == "ban") return await uye.ban({ reason: "Vanir Sikmiş Diyolar" }).catch(err => { })
};
